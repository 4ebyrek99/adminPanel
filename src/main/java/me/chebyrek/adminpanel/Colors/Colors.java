package me.chebyrek.adminpanel.Colors;

import org.bukkit.ChatColor;

public class Colors {
    private static final ChatColor red = ChatColor.RED;
    private static final ChatColor green = ChatColor.GREEN;
    private static final ChatColor gold = ChatColor.GOLD;
    private static final ChatColor darkRed = ChatColor.DARK_RED;
    private static final ChatColor darkBlue = ChatColor.DARK_BLUE;
    private static final ChatColor blue = ChatColor.BLUE;

    public static ChatColor CRed(){
        return red;
    }
    public static ChatColor CGreen(){
        return green;
    }
    public static ChatColor CGold(){
        return gold;
    }
    public static ChatColor CDarkRed(){
        return darkRed;
    }
    public static ChatColor CDarkBlue(){
        return darkBlue;
    }
    public static ChatColor CBlue(){ return blue;}

}
